...
# NEW IMPLEMENTATION CONSTANT HTTP HEADER#
WAVE_SESSION_HEADER = 'Wave-Session-ID'
...
class Auth:
    ...
    async def ensure_fresh_token(self) -> Optional[str]:
        ...
        try:
            ...
            res = await http.get(
                _config.hub_address + '_auth/refresh',
                headers={WAVE_SESSION_HEADER: self._session_id}
            )
            res.raise_for_status()  # Raise an exception for non-successful responses

            # Extract and update tokens if available
            return self.__extract_tokens(res.headers)

        except httpx.HTTPError as e:
            logger.error(f"Failed to refresh OIDC tokens. HTTP error: {e}")
            return None

        except httpx.RequestError as e:
            # Handle request-related errors (e.g., network issues)
            logger.error(f"Failed to make token refresh request: {e}")
            return None
...
class _App:
    def __init__(self, route: str, handle: HandleAsync, mode=None, on_startup: Optional[Callable] = None,
        ...
        # New Implementation ----------------------------------- #
        logger.info(f'Server Mode: {self._mode}')
        logger.info(f'Server Route: {self._route}')
